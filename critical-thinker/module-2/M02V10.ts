...
const getBinnigTimeStamp = (timestamp: string) => {
  const date = new Date(timestamp);
  return Math.floor(date.getTime() / INTERVAL) * INTERVAL;
};

const binningEvent = events?.reduce(
  (events: { [key: string]: { total: number } }, curr) => {
    const bin = getBinnigTimeStamp(curr?.timestamp);
    const binTime = new Date(bin).toISOString();
    ...
    events[binTime].total += 1;
    return events;
  },
  {}
);

console.log(binningEvent);
