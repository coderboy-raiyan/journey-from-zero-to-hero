...
const postsLookup = posts?.reduce(
  (table: { [key: string]: (typeof posts)[0][] }, curr: (typeof posts)[0]) => {
    const { userId } = curr;
    if (!table[userId]) {
      table[userId] = [];
    }

    table[userId].push(curr);
...
const denormalizedUserAndPost = users?.map((user) => {
  return {
    ...user,
    posts: postsLookup[user?.id],
  };
});

console.log(denormalizedUserAndPost);
